...
class InstallManager:
    def __init__(self, bootstrap=False):
        ...
        if bootstrap:
            ...
            target = Path(config['install_dir'])
    ...
    def install_packages(self, packages=None, path=None):
        ...
        if not packages and not path:
            raise Exception(
                'Neither packages nor packages file were specified for installation')
        ...
        if packages:
            if not isinstance(packages, list) and not isinstance(packages, dict):
                raise Exception(
                    'Packages provided are not a list or dictionary')
        ...
        try:
            ...
            logger.error(
                'Packages failed to install because of the following;\n' + str(e))
            raise e
